          ...
          <tbody>
              @foreach ($brands as $item)
                  <tr>
                    ...
                    <td>
                      <a href="{{route('brand.edit', $item->id)}}"  title="edit" data-placement="bottom" style="width: 40px;" class="float-left btn btn-sm btn-outline-warning"> <i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
 
                      <form class="floar-left " action="{{route('brand.destroy', $item->id)}}" method="POST">
                        @csrf
                        @method('delete')
